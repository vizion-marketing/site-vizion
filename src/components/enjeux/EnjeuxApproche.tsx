"use client";

import { useRef, useEffect, useState } from "react";
import { useGSAP } from "@gsap/react";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

interface EnjeuxApprocheItem {
  number: string;
  title: string;
  description: string;
}

interface EnjeuxApprocheProps {
  title: string;
  subtitle: string;
  items: EnjeuxApprocheItem[];
}

export function EnjeuxApproche({ title, subtitle, items }: EnjeuxApprocheProps) {
  const sectionRef = useRef<HTMLElement>(null);
  const [reachedIndex, setReachedIndex] = useState(-1);
  const cardRefs = useRef<(HTMLDivElement | null)[]>([]);
  const [scrollProgress, setScrollProgress] = useState(0);

  // Track highest reached card via IntersectionObserver
  useEffect(() => {
    const observers: IntersectionObserver[] = [];
    cardRefs.current.forEach((ref, i) => {
      if (!ref) return;
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            setReachedIndex((prev) => Math.max(prev, i));
          }
        },
        { threshold: 0.4 },
      );
      observer.observe(ref);
      observers.push(observer);
    });
    return () => observers.forEach((o) => o.disconnect());
  }, [items.length]);

  // Scroll progress for the vertical bar
  useEffect(() => {
    const handleScroll = () => {
      if (!sectionRef.current) return;
      const rect = sectionRef.current.getBoundingClientRect();
      const progress = Math.min(
        Math.max(-rect.top / (rect.height - window.innerHeight), 0),
        1,
      );
      setScrollProgress(progress);
    };
    window.addEventListener("scroll", handleScroll, { passive: true });
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  useGSAP(
    () => {
      gsap.from("[data-approche='title']", {
        opacity: 0,
        y: 30,
        duration: 0.8,
        ease: "power3.out",
        scrollTrigger: { trigger: sectionRef.current, start: "top 80%" },
      });
      gsap.fromTo(
        "[data-approche='card']",
        { opacity: 0, x: 80 },
        {
          opacity: 1,
          x: 0,
          duration: 0.8,
          stagger: 0.15,
          ease: "power3.out",
          scrollTrigger: { trigger: "[data-approche='cards']", start: "top 85%" },
        },
      );
    },
    { scope: sectionRef },
  );

  return (
    <section
      ref={sectionRef}
      className="dark-section grain-overlay relative py-16 sm:py-20 md:py-24 lg:py-28 px-4 sm:px-6 md:px-12"
      style={{ background: "var(--bg-dark)" }}
    >
      {/* Accent blobs */}
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div
          className="absolute w-[50%] h-[50%] top-[10%] right-[-10%]"
          style={{
            background:
              "radial-gradient(ellipse 100% 100% at 50% 50%, rgba(var(--color-accent-rgb), 0.08) 0%, transparent 55%)",
          }}
        />
        <div
          className="absolute w-[40%] h-[40%] bottom-[5%] left-[-5%]"
          style={{
            background:
              "radial-gradient(ellipse 100% 100% at 50% 50%, rgba(var(--color-accent-rgb), 0.06) 0%, transparent 55%)",
          }}
        />
      </div>

      <div className="max-w-[82.5rem] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-10 lg:gap-16 relative z-10">
        {/* Left — Sticky title panel (remplace l'image) */}
        <div className="lg:sticky lg:top-[100px] lg:self-start">
          <div data-approche="title" className="relative overflow-hidden">
            <div className="flex items-center gap-2.5 mb-5">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-accent opacity-40" />
                <span className="relative inline-flex rounded-full h-2 w-2 bg-accent" />
              </span>
              <span className="text-[10px] sm:text-[11px] font-light tracking-[0.12em] text-white/60 uppercase">
                Notre approche
              </span>
            </div>

            <h2 className="font-heading font-medium text-[28px] sm:text-[36px] md:text-[44px] leading-[1.05] tracking-[-0.02em] text-white mb-5">
              {title}
            </h2>

            <p className="text-[15px] text-white/60 leading-relaxed mb-10 max-w-sm">
              {subtitle}
            </p>

            {/* Scroll progress bar */}
            <div className="h-[2px] bg-white/[0.08] max-w-[200px]">
              <div
                className="h-full bg-accent origin-left"
                style={{ transform: `scaleX(${scrollProgress})`, transition: "none" }}
              />
            </div>

            {/* Numéro décoratif */}
            <div
              aria-hidden="true"
              className="absolute bottom-[-50px] right-0 text-[140px] font-bold text-white/[0.03] leading-none select-none pointer-events-none"
            >
              0{items.length}
            </div>
          </div>
        </div>

        {/* Right — Scrollable numbered cards */}
        <div data-approche="cards" className="flex flex-col">
          <div className="relative lg:pl-5 flex flex-col gap-4 sm:gap-5">
            {items.map((item, i) => {
              const isReached = i <= reachedIndex;
              return (
                <div key={i}>
                  {i > 0 && <div className="h-px bg-white/[0.08]" />}
                  <div
                    ref={(el) => { cardRefs.current[i] = el; }}
                    data-approche="card"
                    className={`py-7 sm:py-8 px-6 sm:px-8 transition-all duration-500 ${
                      isReached
                        ? "bg-white/[0.06] backdrop-blur-sm"
                        : "hover:bg-white/[0.03]"
                    }`}
                  >
                    <div className="flex items-start gap-5">
                      <span
                        className={`text-[13px] font-medium tracking-wide shrink-0 mt-1 transition-colors duration-500 ${
                          isReached ? "text-accent" : "text-white/30"
                        }`}
                      >
                        {item.number}
                      </span>
                      <div>
                        <h3 className="text-[17px] sm:text-[19px] font-semibold text-white leading-snug mb-2">
                          {item.title}
                        </h3>
                        <p className="text-[14px] sm:text-[15px] text-white/60 leading-relaxed">
                          {item.description}
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              );
            })}

            {/* Barre de progression verticale */}
            <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-white/[0.08] hidden lg:block">
              <div
                className="w-full bg-accent origin-top"
                style={{ height: `${scrollProgress * 100}%`, transition: "none" }}
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
